...
def normalise_map_dict(mapping_dict, old_cats, new_cats, map_is_id):
    ...
    if map_is_id:
        new_map_dict = { old_id_to_name[int(old_id)] : new_id_to_name[int(new_id)] for old_id, new_id in mapping_dict.items() }
        return new_map_dict
    else:
...
def map_cat_from_files(coco_json, new_cat_json, mapping_json, out_json=None, keep_old=False, map_is_id=False):
    coco_dict, _ = read_coco_json(coco_json)
    new_cat_dict = read_json(new_cat_json)
    mapping_dict = read_json(mapping_json)

    out_dict = map_cat(coco_dict, new_cat_dict, mapping_dict, keep_old=keep_old, map_is_id=map_is_id)
...
def map_cat(coco_dict, new_cat_dict, mapping_dict, keep_old=False, map_is_id=False):
    ...
    mapping_dict = normalise_map_dict(mapping_dict, coco_dict['categories'], new_cat_list, map_is_id)
