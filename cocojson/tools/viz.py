...
from cocojson.utils.common import path, read_coco_json, get_img2annots, get_flatten_name, get_imgnames_dict
...
def viz_individual_box(json, root, outdir, color=(255,255,0), thickness=1, buffer=0):
    ...
    for annot in tqdm(coco_dict['annotations']):
        ...
        img_show = img.copy()
        l,t,r,b = draw_annot(img_show, annot, color=color, thickness=thickness, buffer=buffer) 
        ship_idx = imgid2shipcount[annot['image_id']] - 1
        out_img_path = outdir / f'{img_path.stem}_ship{ship_idx}_ltrb{int(l)},{int(t)},{int(r)},{int(b)}.jpg'
